...
- `kurokami/core.py` — Pure library (keeps the Carousell HTML layout docstring): URL building (search sort order via `SORT_OPTIONS`: best_match/recent/price_asc/price_desc/nearby), blacklist, `parse_info`, `find_item_divs`/`parse_items` (keeps the first row per uid; `make_valid_counter` builds the browser-loop counter), high-level `async scrape()` (returns a DataFrame, never writes output), `new_rows()` diff.
...
- `kurokami/cli.py` — `async def main(options)` preserving CLI behaviour: argparse/menu, interactive prompts, `utf-8-sig` CSV output, `sys.exit(1)` on no-results. The dict form is a server-side API used by the (future) bot, with args `i`, `n`, `o`, `t`, `s`, `c` (`ph`/`pl`/`so` optional; `so` = sort name or raw Carousell value).
...
- `server.py` — aiohttp app (tickets 005 + 008): bound to 127.0.0.1, serves `static/`, validates browser inputs (SSRF note), and drives the daemon. Endpoints: `GET /api/watches`, `POST /api/watches`, `GET/DELETE /api/watches/{id}`, `PATCH /api/watches/{id}` (edit query/schedule; a query change resets the diff baseline and requeues a scan), `POST /api/watches/{id}/retry`, `POST /api/watches/{id}/rescan`, `GET /api/watches/{id}/results`, `GET /api/feed`, `GET /api/status`, plus the shell seam `POST /api/search` + `GET /api/search/{job_id}` + `GET /api/jobs` + `GET /api/config`. `KUROKAMI_SERVER_TEST=1` makes scrapes read `utils/soup.pkl` instead of driving Chrome.
...
- Run: `python kurokami.py -i "<item>" -n <count> [-o out.csv] [-ph H] [-pl L] [--sort recent|best_match|price_asc|price_desc|nearby] [-t|-s] [-c prev.csv]` (or `python -m kurokami ...`).
...
- Smoke-check the server: `python test_server.py` (in-process, patched scrape; 54 checks).
